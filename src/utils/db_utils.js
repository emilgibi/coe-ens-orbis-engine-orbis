...
export async function updatedinsertTable2(tableName, data, primaryId) {
  if (
    !tableName ||
    typeof data !== 'object' ||
    Object.keys(data).length === 0
...
export async function doesDataExist(primaryId) {
  if (!primaryId) {
    throw new Error('Invalid table name or primary ID');
  }
  const tableName = 'grid_pm_tracking'
  const query = `SELECT 1 FROM ${tableName} WHERE primary_id = $1 LIMIT 1`;
  ...
  try {
    ...
    return false; // Optional: throw error instead if needed
  }
}
...
export async function checkExistingRecord(tableName, bvd_id) {
  if (!tableName || !bvd_id) {
    throw new Error('Invalid table name or bvd_id');
  }
  ...
  try {
    const result = await pool.query(query, [bvd_id]);
    if (result.rowCount === 0) {
