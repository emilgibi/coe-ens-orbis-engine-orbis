...
export const getAddressImages = async (req, res) => {
    const { address, bvd_id, sessionId, ensId } = req.query;
    const orgName = req.query.orgName || req.query.name || null;

    // Request-level context object so every log carries the same bvd_ids.
    ...
    try {
        ...
        ───────────────────────────────────────────────────────────── */
        if (!address || !bvd_id || !sessionId || !ensId) {
        ...
        let matchedRow = null;
        try {
            matchedRow = await findMatchingAddressZone(bvd_id, lat, lng);
        ...
        const imageName = `${bvd_id}_${imageCode}`;
...
export const getPlaceDetails = async (req, res) => {
    const { name, address, bvd_id} = req.query;
    console.info("[Places] Request received", { bvd_id, name });

    try {
        console.info("[Places] Checking DB cache");
        const existing = await checkExistingRecord("google_ratings", bvd_id);

        if (existing.exists) {
            console.info("[Places] Cache hit", { bvd_id });
            return res.status(200).json({
                success: true,
                source: "cache",
                data: existing.data,
        ...
        console.info("[Places] Completed successfully", { bvd_id });
